  ...
  const handleVote = () => {
    ...
    axios.post(`${getBaseUrl()}/users`, {
      ...
      .then((res) => {
        ...
        axios.post(`${getBaseUrl()}/polls/${slug}/vote/${userId}`, {choices})
          .then((voteRes) => {
            refreshDataAndUsers()
            setUsername("")
            setChoices([])
        ...
        if(mealPreferences.trim() !== "") {
          axios.post(`${getBaseUrl()}/polls/${slug}/mealpreference/${userId}`, {content: mealPreferences})
            .then(res => {
...
const Comments = ({data, username, setUsername, slug}) => {
  ...
  const createComment = () => {
    axios.post(`${getBaseUrl()}/users`, {
      ...
      .then((res) => {
        let {id:  userId} = res.data
        axios.post(`${getBaseUrl()}/polls/${slug}/comments/${userId}`, {content: comment})
          .then(res => {
            setComment("")
            setComments([
              ...comments,
...
const Poll = (props) => {
  ...
  const refreshDataAndUsers = () => {
    ...
    axios.get(`${getBaseUrl()}/polls/${slug}/users`)
    .then((res) => {
      setUsers(res.data)
    })
    .catch((err) => {
      ...
      { isModalOpened &&
        <div className="modal" onClick={() => setIsModalOpened(false)}>
          <div className="Export_Modal" >
            ...
            <a className="Export" target="_blank" rel="noopener noreferrer" href={`${getBaseUrl()}/polls/${slug}/results`}>
